...
jaqpot.predict.rpart <- function(dataset, rawModel, additionalInfo){
  ...
  if (model$method == "class"){
      # Predict function on rpart returns the probabilities for each class. So obtain the probs and return the class
      # with the highest probs
      predictions_raw <- predict(model, df)
